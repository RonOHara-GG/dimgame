...
void DBIdentifyHandler(kpnBaseSocket* pSocket, kpnPacketInfo* pPacket)
{
	kpnGPIdentity identity;
	identity.SetAddress(pSocket->GetLocalIP());
	identity.SetPort(DATABASE_SERVER_PORT);
	...
	pSocket->Send(&identity, pPacket->m_unAddress, pPacket->m_nPort, false);
}

void DBConnectHandler(kpnBaseSocket* pSocket, kpnPacketInfo* pPacket)
{
...
DispatchFunction s_pPacketHandlers[] = {
	/* GPID_IDENTITY */		0,
	/* GPID_IDENTIFY */		DBIdentifyHandler,
	/* GPID_CONNECT */		DBConnectHandler,
	/* GPID_CONNECTED */	0,
};